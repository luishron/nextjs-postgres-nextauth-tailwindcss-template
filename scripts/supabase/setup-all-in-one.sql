...
CREATE POLICY IF NOT EXISTS "Users can view their own income categories"
  ON income_categories FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can insert their own income categories"
  ON income_categories FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update their own income categories"
  ON income_categories FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can delete their own income categories"
  ON income_categories FOR DELETE USING (auth.uid() = user_id);

-- Agregar foreign key a incomes después de crear income_categories
ALTER TABLE incomes
  DROP CONSTRAINT IF EXISTS incomes_category_id_fkey,
